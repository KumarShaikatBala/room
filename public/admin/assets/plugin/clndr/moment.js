...
return m;}
function warn(msg){if(utils_hooks__hooks.suppressDeprecationWarnings===false&&typeof console!=='undefined'&&console.warn){}}
...
function getSetWeek(input){var week=this.localeData().week(this);return input==null?week:this.add('d',(input-week)*7);}
function getSetISOWeek(input){var week=weekOfYear(this,1,4).week;return input==null?week:this.add('d',(input-week)*7);}
...
function getSetDayOfYear(input){var dayOfYear=Math.round((this.clone().startOf('day')-this.clone().startOf('year'))/864e5)+1;return input==null?dayOfYear:this.add('d',(input-dayOfYear));}
...
function createFromConfig(config){var res=new Moment(checkOverflow(prepareConfig(config)));if(res._nextDay){res.add('d',1);res._nextDay=undefined;}
return res;}
...
this._offset=input;this._isUTC=true;if(localAdjust!=null){this.add('m',localAdjust);}
...
(other.year()-base.year())*12;if(base.clone().add('M').isAfter(other)){--res.months;}
