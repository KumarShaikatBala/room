                        ...
                        <div class="dataTables_wrapper">
                            <div class="table-responsive">
                                <table class="table table-hover dataTable table-responsive" data-table="table-button" data-buttons="['copy', 'csv', 'excel', 'pdf', 'print']" style="overflow: auto!important;">
                                    <h5 class="text-info">Total PayableAmount <i class="fa fa-angle-double-right fa-lg text-info" aria-hidden="true"></i>  {{$payments->sum('amount')}}<span class="ti-money"></span></h5>
                                    <h5 class="text-info">Total Paid <i class="fa fa-angle-double-right fa-lg text-info" aria-hidden="true"></i>  {{$cash->sum('amount')}}<span class="ti-money"></span></h5>
                                    ...
                                    @foreach($payments as $payment)
                                        <tr>
                                            ...
                                            @if ($payment->status===null)
                                                ...
                                                @else
                                                    <span class="label label-outline-success">Paid</span>
                                            @endif
                                            </td>
                                            <td class="text-center">{{$payment->rooms->type}}</td>
                                            <td>
                                                @if ($payment->status===null)
                                                    <a href="{{route('cash_receive',['id'=>$payment->id,'booking'=>$payment->booking->id])}}" class="btn btn-outline-success btn-xs"><i class="fa fa-check" aria-hidden="true"></i></a>
                                                @else
@endif
                                                ...
                                                <a href="{{route('cash_destroy',['id'=>$payment->id])}}" class="btn btn-outline-danger btn-xs" style="float: right"><i class="fa fa-trash" aria-hidden="true"></i><i class="fa fa-remove" aria-hidden="true"></i></a>
                                            </td>
                                        </tr>
                                    @endforeach
                                    </tbody>
